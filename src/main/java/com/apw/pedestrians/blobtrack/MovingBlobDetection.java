...
public class MovingBlobDetection implements IMovingBlobDetection {
  ...
  public List<MovingBlob> getUnifiedBlobs(List<MovingBlob> movingBlobs) {
    ...
    // second dimension is length 3 because we are storing [distance, index of first point, index
    // of second point]
    ...
    for (int i = 0; i < distances.length; i++) {
      ...
      if (pointSet1 != pointSet2 || pointSet1 == null) {
        if (pointSet1 == null && pointSet2 == null) {
          if (distBetweenPoints(finalPoints[point1], finalPoints[point2])
              <= maxDistBetweenPointsInCluster) {
          ...
          for (int point : pointSet2) {
            if (distBetweenPoints(finalPoints[point1], finalPoints[point])
                > maxDistBetweenPointsInCluster) {
              canCombine = false;
            }
          }
          ...
          for (int point : pointSet1) {
            if (distBetweenPoints(finalPoints[point2], finalPoints[point])
                > maxDistBetweenPointsInCluster) {
              canCombine = false;
            }
          }
          ...
          for (int points1 : pointSet1) {
            for (int points2 : pointSet2) {
              if (distBetweenPoints(finalPoints[points1], finalPoints[points2])
                  > maxDistBetweenPointsInCluster) {
                canCombine = false;
              }
            }
  ...
  private float distBetweenBlobs(float[] point, MovingBlob blob1, MovingBlob blob2) {
    ...
    return (float) Math.sqrt(distanceX * distanceX + distanceY * distanceY + distanceVX * distanceVX
        + distanceVY * distanceVY);
  }

  private float distBetweenPoints(float[] point1, float[] point2) {
    return (float) Math.sqrt(Math.pow(xDistWeight * (point1[0] - point2[0]), 2) + Math
        .pow(yDistWeight * (point1[1] - point2[1]), 2) +
        Math.pow(vXWeight * (point1[2] - point2[2]), 2) + Math
        .pow(vYWeight * (point1[3] - point2[3]), 2));
  }
  ...
  public void updateFilteredUnifiedBlobs(List<MovingBlob> blobList) {
    ...
    for (int i = 0; i < pairs.length; i++) {
      ...
      float percentWChange = Math.abs(pairs[i][0].width - pairs[i][1].width) / Math
          .max(pairs[i][0].width, pairs[i][1].width);
      float percentHChange = Math.abs(pairs[i][0].width - pairs[i][1].width) / Math
          .max(pairs[i][0].width, pairs[i][1].width);
