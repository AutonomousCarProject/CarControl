...
import static com.apw.pedestrians.blobdetect.PrimitiveBlob.BLOB_BOTTOM;
import static com.apw.pedestrians.blobdetect.PrimitiveBlob.BLOB_COLOR;
import static com.apw.pedestrians.blobdetect.PrimitiveBlob.BLOB_IS_UNMATCHED;
import static com.apw.pedestrians.blobdetect.PrimitiveBlob.BLOB_LEFT;
import static com.apw.pedestrians.blobdetect.PrimitiveBlob.BLOB_MATCHED;
import static com.apw.pedestrians.blobdetect.PrimitiveBlob.BLOB_NUM_INT_FIELDS;
import static com.apw.pedestrians.blobdetect.PrimitiveBlob.BLOB_REFERENCE_INDEX;
import static com.apw.pedestrians.blobdetect.PrimitiveBlob.BLOB_RIGHT;
import static com.apw.pedestrians.blobdetect.PrimitiveBlob.BLOB_TOP;
import static com.apw.pedestrians.blobdetect.PrimitiveBlob.BLOB_TYPE;
import static com.apw.pedestrians.blobdetect.PrimitiveBlob.BLOB_TYPE_NULL;
import static com.apw.pedestrians.blobdetect.PrimitiveBlob.BLOB_TYPE_REFERENCE;
import static com.apw.pedestrians.blobdetect.PrimitiveBlob.BLOB_TYPE_VALUE;

public class NonGPUPrimitiveBlobDetection extends BlobDetection {
    ...
    Kernel rightCheckKernel = null, bottomCheckKernel = null;
    ...
    public List<Blob> getBlobs(Pixel[][] pixels) {
            ...
            for (int col = 0; col < width - 1; col++) {
                ...
                if (color1 == color2) {
                    ...
                    if (blobs[blob1 + BLOB_TYPE] == BLOB_TYPE_NULL) {
                        ...
                        blobs[blob1 + BLOB_RIGHT] = Math.max(blobs[blob1 + BLOB_RIGHT], col + 1);
                        ...
                        blobs[tlBlob + BLOB_RIGHT] = Math.max(blobs[tlBlob + BLOB_RIGHT], col + 1);
                    }
        ...
        for (int col = 1; col < height; col += 2) {
            for (int row = 0; row < height - 1; row++) {
                int color1 = colors[(row * width) + col];
                int color2 = colors[((row + 1) * width) + col];
        ...
        */
        ...
        for (
                int i = 0;
                i < blobs.length; i += BLOB_NUM_INT_FIELDS)
