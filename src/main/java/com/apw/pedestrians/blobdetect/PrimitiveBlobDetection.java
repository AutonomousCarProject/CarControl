...
public class PrimitiveBlobDetection implements IBlobDetection {
  ...
  private static final int BIP_TYPE = 0, BIP_TOP = 1, BIP_LEFT = 2, BIP_BOTTOM = 3, BIP_RIGHT =
      4, BIP_COLOR = 5, BIP_NUMFIELDS = 6;
        ...
        for (int col = 0; col < pixels[0].length; col++) {
          ...
          if (color1 == color2) {
            if (bips[bip1 + BIP_TYPE] != BIP_TYPE_NULL
                ...
                && abs(bip2Width - bip1Width)
                <= MAXIMUM_DIFFERENCE_IN_WIDTH_BETWEEN_TWO_BLOBS_IN_ORDER_TO_JOIN) {
            ...
            // Potentially re-add these later, but according to the original algorithm, these
            // will not be called
  ...
  private Blob getBlob(int bip) {
    ...
    if (unusedBlobs.isEmpty()) {
      return new Blob(bipWidth, bipHeight, bips[bip + BIP_LEFT], bips[bip + BIP_TOP],
          new Pixel(bipColor));
    } else {
