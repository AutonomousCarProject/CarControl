...
public class DrDemo extends JFrame implements MouseListener, KeyListener {
  ...
  public void TestServos() { // exercise steering & ESC servos
    ...
    while (true) {
      ...
      if (!flash) {
        theSim.SeeOnScrnPaint(ServoMsgTL >> 16, ServoMsgTL & 0xFFFF,
  ...
  public void paint(Graphics graf) { // (in DrDemo)
    ...
    if (thePixels != null) {
      try {
        ...
        if (GetCameraImg()) {
          ...
          //graf.drawRect(Constants.STOPLIGHT_MIN_X, Constants.STOPLIGHT_MIN_Y, Constants
          // .STOPLIGHT_MAX_X-Constants.STOPLIGHT_MIN_X, Constants.STOPLIGHT_MAX_Y-Constants
          // .STOPLIGHT_MIN_Y);

          TestServos(); // (replace this with your own code)

          graf.setColor(Color.RED);
          //graf.drawRect(Constants.STOPSIGN_MIN_X, Constants.STOPSIGN_MIN_Y, Constants
          // .STOPSIGN_MAX_X-Constants.STOPSIGN_MIN_X, Constants.STOPSIGN_MAX_Y-Constants
          // .STOPSIGN_MIN_Y);
    ...
    if (DriverCons.D_DrawCurrent == true) {
      for (int i = 0; i < testSteering.startingPoint - (testSteering.startingHeight
          + testSteering.heightOfArea); i++) {
        graf.fillRect(testSteering.leadingMidPoints[i].x,
            testSteering.leadingMidPoints[i].y + +edges.top, 5, 5);
      }
    }
    ...
    for (int i = 0; i < hi.length; i++) {
      ...
      if (DriverCons.D_DrawOnSides == true) {
        ...
        graf.fillRect(testSteering.leftPoints[i].x + edges.left,
            testSteering.leftPoints[i].y + edges.top, 5, 5);
        graf.fillRect(testSteering.rightPoints[i].x + edges.left,
            testSteering.rightPoints[i].y + edges.top, 5, 5);
      }
    }
  } //~paint
