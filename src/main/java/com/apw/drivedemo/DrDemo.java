...
public class DrDemo extends JFrame implements MouseListener, KeyListener {
    ...
    private static final int MinESCact = DriverCons.D_MinESCact, MaxESCact = DriverCons.D_MaxESCact,
            StartGas = MinESCact * 9 / 4, ScrPix = TrakSim.nPixels, nServoTests = DriverCons.D_nServoTests,
            ...
            FastSet = (MaxESCact + MinESCact) / 2, ScrWi = TrakSim.WinWi, ScrFrTime = DriverCons.D_FrameTime,
            Scr2L = ScrWi * 2, CamFPS = FlyCamera.FrameRate_15 - 1, // CamTime = 528>>CamFPS, // cam frame in ms

    DrawDash = DriverCons.D_DrawDash, CarColo = DriverCons.D_CarColo, AddColo = DriverCons.D_MarinBlue,
    ...
    private static final boolean StartInCalibrate = DriverCons.D_StartInCalibrate, ContinuousMode = false,
    ...
    private DriveTest dtest = new DriveTest(4);
            ...
            System.out.println(HandyOps.TF2Log("** GetCamIm ", (myVid != null),
                    HandyOps.TF2Log("/", myBy != null, HandyOps.TF2Log("/", myPix != null,
                            HandyOps.TF2Log(" = ", gotit, HandyOps.Dec2Log(" ** ", wide, // gotit =
                                    HandyOps.Dec2Log("/", ScrWi, HandyOps.TF2Log(" ", alive, HandyOps.Dec2Log("/", whom,
                ...
                HandyOps.Dec2Log(" @ ", Vx,
                        HandyOps.Dec2Log(",", Hx, HandyOps.Int2Log(": ", nx, HandyOps.Dec2Log(" ", StartYourEngines,
                                HandyOps.TF2Log(" g=", unPaused, HandyOps.TF2Log(" cv=", CameraView, HandyOps.Dec2Log(
                                        " ns=", NoneStep,
                                        HandyOps.Dec2Log(" ", Calibrating, HandyOps.PosTime(((" @ ")))))))))))));
    } // ~mouseEntered
    ...
    public void mouseClicked(MouseEvent evt) { // (in DrDemo)
        ...
        System.out.println(HandyOps.Dec2Log("(DrDemo) Got click @ ", Vx,
                HandyOps.Dec2Log(",", Hx, HandyOps.Dec2Log(": ", nx, HandyOps.Dec2Log("/", zx,
                        HandyOps.Dec2Log(" +", kx, HandyOps.Dec2Log(" ", StartYourEngines, HandyOps.TF2Log(" s=", true,
    ...
    public void TestServos() { // exercise steering & ESC servos
        ...
        boolean nowate = (nServoTests > 99) && ((nServoTests & 1) != 0), doit = (StartYourEngines != 0) || nowate,
                flash = doit;
        ...
        while (true) {
            ...
            if (flash)
                if ((ServoTestCount & 1) == 0)
                    flash = false;
            if (!flash)
                theSim.SeeOnScrnPaint(ServoMsgTL >> 16, ServoMsgTL & 0xFFFF, ServoMsgSiz >> 16, ServoMsgSiz & 0xFFFF,
                        ServoTstPos, -1);
            if (!CameraView)
                break;
            why++; // why = 9
    ...
    public void DrawDemo() { // examples of drawing on TrakSim image..
        int haff = ScrHi / 2, tx = theSim.TurnRadRow(), t2x = tx >> 16, HafScrn = ImgWi / 2, nx, zx, whar = 0;
