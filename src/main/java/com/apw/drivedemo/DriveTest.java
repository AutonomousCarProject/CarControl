...
public class DriveTest extends JFrame implements MouseListener {
  ...
  public static void main(String[] args) {
    ...
    displayTaskTimer
        .scheduleAtFixedRate(new TimerRepaint(new DriveTest(3)  //IMPORTANT		set the viewType here
        ) {
          @Override
          public void run() {
    ...
    displayTaskTimer.scheduleAtFixedRate(
        new TimerRepaint(new DriveTest(2, 640, 480)  //IMPORTANT		set the viewType here
        ) {
          @Override
          public void run() {
            window.sim.SimStep(1);
  ...
  public void mouseClicked(MouseEvent evt) {
    ...
    if (Hx < ImgWi) {
      ...
      if (nx < 3) { // top half, switch to camera view..
        ...
        if (didit) {
          sim.DarkFlash(); // unseen if switches to live cam
        }
        ...
        DidFrame = 0;
        if (sim.IsCrashed()) {
          sim.SimStep(0); // clear crashed mode
        }
        if (zx == 2) {
      ...
      if ((Vx < (zx >> 16)) && (nx < (zx & 0xFFF))) {
        ...
        zx = sim.ZoomMap2true(true, Vx, Hx); // sets facing to -> click
      }
