...
public class DriveTest extends JFrame {
  ...
  public static void main(String[] args) {
    ...
    //displayTaskTimer.scheduleAtFixedRate(new TimerRepaint(createDriveTest(3)), new Date(), 1000
    // / FPS);
  }
    ...
    if (DriverCons.D_DrawCurrent == true) {
      for (int i = 0; i < testSteering.startingPoint - (testSteering.startingHeight
        ...
        graf.fillRect(testSteering.leadingMidPoints[i].x,
            testSteering.leadingMidPoints[i].y + +edges.top + vEdit, 5, 5);
      }
    }
    ...
    for (int i = 0; i < hi.length; i++) {
      ...
      if (DriverCons.D_DrawOnSides == true) {
        ...
        graf.fillRect(testSteering.leftPoints[i].x + edges.left,
            testSteering.leftPoints[i].y + edges.top + vEdit, 5, 5);
        graf.fillRect(testSteering.rightPoints[i].x + edges.left,
            testSteering.rightPoints[i].y + edges.top + vEdit, 5, 5);
      }
    }
  } //~steerPaint
  ...
  private void speedPaint(Graphics graf) {
    ...
    ImageManipulator
        .limitTo(limitArray, imageManager.getSimpleColorRaster(), ncols, nrows, 640, 480, false);
