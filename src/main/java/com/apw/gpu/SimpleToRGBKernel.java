 ...
 * The raster is comprised of <code>RED</code>, <code>GREEN</code>, <code>BLUE</code>,
 * <code>WHITE</code>, and <code>GREY</code>
...
public class SimpleToRGBKernel extends Kernel {
  ...
  /**
   * Constructs an <code>SimpleToRGBKernel</code> Aparapi
  ...
  public void run() {
    ...
    } else if (simpleByte[i] == 2) {
      simpleRGB[i] = 0x0000FF;
    } else if (simpleByte[i] == 3) {
      simpleRGB[i] = 0xFFFFFF;
    } else if (simpleByte[i] == 4) {
      simpleRGB[i] = 0x808080;
    } else if (simpleByte[i] == 5) {
