...
public class ImageManagementModule implements Module {
    ...
    public void removeShadows(){
        ...
        for(int r = height-2;r>0;r--){
            for(int c = frameWidth-2; c>frameWidth/2;c--){
                int side=1;
                Y = (R[r*width+c]&0xFF) *  .299000 + (G[r*width+c]&0xFF) *  .587000 + (B[r*width+c]&0xFF) *  .114000;
                Y1 = (R[r*width+c+side]&0xFF) *  .299000 + (G[r*width+c+side]&0xFF) *  .587000 + (B[r*width+c+side]&0xFF) *  .114000;
                ...
                U1 = (R[r*width+c+side]&0xFF) * -.168736 + (G[r*width+c+side]&0xFF) * -.331264 + (B[r*width+c+side]&0xFF) *  .500000 + 128;
                ...
                V1 = (R[r*width+c+side]&0xFF) *  .500000 + (G[r*width+c+side]&0xFF) * -.418688 + (B[r*width+c+side]&0xFF) * -.081312 + 128;
