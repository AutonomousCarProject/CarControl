...
public class HandyOps { // first some useful debug logging ops..
  ...
  public static String Hex2Log(String before, int whom, int nx, String after) {
    ...
    if (nx > 0) {
      ...
      if (nx > 0) {
        return Hex2Log(before, whom, nx, after);
      }
    }
    return before + after;
  } //~Hex2Log
  ...
  public static String Int2Log(String before, int whom, String after) {
    ...
    if (info == whom) {
      return Dec2Log(before, whom, after);
    }
  ...
  public static String ReplacAll(String aStr, String xStr, String whom) {
    ...
    try {
      ...
      while (true) {
        ...
        if (here > 0) {
          whom = whom.substring(0, here) + aStr
              + whom.substring(here + lxx);
        } else if (aStr == "") {
          whom = whom.substring(here + lxx);
        } else {
          whom = aStr + whom.substring(here + lxx);
        }
        thar = thar + more;
      }
    } //~while //~try
  ...
  public static int ReadTiff32Image(String filename, int[] pixels) {
    ...
    try {
      while (true) {
        ...
        why++; // why = 7
        if (xData[0] == 0x49) {
          LilEndian = true; // = 'I' (Intel/inverted)
        } else if (xData[0] != 0x4D) {
          break; // 'M' (Motorola/Mac)
        }
        if (LilEndian) {
          info = xData[2];
        } else {
        ...
        if (info != 0x2A) {
          break; // =42 (otherwise not Tiff)
        }
        if (!markt) { // gotta read the index..
          why++; // why = 8
          ixat = Int4bytes(LilEndian, xData[4], xData[5], xData[6], xData[7]);
          if (ixat < 8) {
            break; // invalid start of index
          }
          if (ixat + 32 > lxx) {
            break; // didn't get enough to use
          }
          if (ixat > 555) {
            break; // we don't do files with index at end
          }
          why++; // why = 9
          here = ixat - 2;
          if (here < 0) {
          ...
          if (LilEndian) {
            ...
            tops = MyMath.SgnExt(tops);
          }
          if (tops < 4) {
            break; // didn't get enough index to use
          } else if (tops * 12 + 12 > lxx) {
            break; // index bigger than what we got
          }
          why++; // why = 11
          here = ixat + 2;
          for (nx = tops - 1; nx >= 0; nx += -1) { // extract Tiff info..
            ...
            if (info == 256) { // ImageWidth
              ...
              if (logy) {
                aStr = " PhotometricInterpretation=2";
              }
              zx = Int4bytes(LilEndian, xData[here], xData[here + 1], xData[here + 2],
                  xData[here + 3]);
              if ((zx == 0) || ((zx & ~0x02020002) != 0)) {
                why = why | 512;
              }
            } //~if
              ...
              why = why | 128; // ColorMap (should be none)
            }
            if (logy) {
              System.out.println(Dec2Log("  @@ ", thar, Int2Log(": ", info,
        ...
        Dec2Log(" ", tall, Dec2Log("/", wide, Dec2Log(" ", lxx,
            IffyStr((why == 0) && !NoisyMap, filename, Dec2Log(" #", tops,
                Dec2Log(" @ ", ixat, Dec2Log(" s", samp, TF2Log(" ", LilEndian,
                    Dec2Log(" @ ", whar, Int2Log(aStr, why, filename)))))))))
        ))); // why =
    if (why != 0) {
      TifFileNm = "";
      TifDataOfx = 0;
  ...
  public static String Fixt8th(String before, int whom, String after) {
    try {
      ...
      if (whom < 0) {
        return before + Fixt8th("-", -whom, after);
      }
      ...
      if ((whom & 1) == 0) {
        return before + Dec2Log(".", (whom >> 1) * 25, after);
      }
