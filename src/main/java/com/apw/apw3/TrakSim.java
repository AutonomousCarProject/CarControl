...
import com.apw.pinio.fakefirm.SimHookBase;
import java.io.File;
...
public class TrakSim {
  ...
  private final int[] Grid_Locns = {6, 12, 16, 18, 26, 29, // (GridLocT) block index
      ...
      0, ImWi / 7, ImWi * 2 / 7, ImWi * 3 / 7, ImWi * 4 / 7, ImWi * 5 / 7, ImWi * 6 / 7, ImWi,
      // mid
  ...
  public static int EncoPavEdge(int tall, int wide, int recell, int why) {
    ...
    if ((MyMath.iAbs(tall) > MyMath.iAbs(wide)) || (why < 0)) { // more N-S than E-W,
      ...
      if (tall < 0) {
        bitz = bitz | 0x40000000; // going north, paved to west (<)
      } else {
        grtr = true; // otherwise going south, paved to east (>)
      }
      ...
      if (tall == wide) {
        ...
        Mconst = Mconst / tall; // (..so this is float divide)
      }
      ...
      if (wide > 0) {
        bitz = bitz | 0x40000000; // going east, paved to north (<)
      } else {
        grtr = true; // otherwise going west, paved to south (>)
      }
    ...
    eps = Cast_F2I(Mconst);
    if (eps >= 0x1000) {
      Mconst = Cast_I2F(0xFFF);
    } else if (why != 0) {
      if (eps + 1 == 0) {
        Mconst = Cast_I2F(eps - 1);
      }
    }
  ...
  public static void WhiteAlfa(int tall, int wide, int[] theImgs) {
    ...
    if (tall < 4) {
      return; // not a credible image array
    }
    if (wide < 4) {
      return;
    }
  ...
  private static int Ad2PntMap(int rx, int cx, int img,
    ...
    while (true) {
      ...
      if (thar > myMap.length - 4) {
        break;
      }
      why++; // why = 6
      if (myMap[thar + 2] != 0) {
        break; // last error exit
      }
      why++; // why = 7
      if (more == 0) {
        more++; // 1x1 pix image allowed, but don't fail 0-test
      }
      myMap[thar] = img;
  ...
  private static void MapBucket(int deep, int rx, int cx,
    ...
    while (true) {
      ...
      if (info == 0) {
        MapBucket(deep + 1, rx, cx + 1, colo, myMap);
      }
      info = myMap[thar - 1];
      if (info != colo) {
        if (info == bitz) {
      ...
      if (thar > myMap.length - (HalfMap + 2)) {
        break;
      }
      if (thar < HalfMap) {
        break;
      ...
      if (info == 0) {
        MapBucket(deep + 1, rx + 1, cx, colo, myMap);
      }
      info = myMap[thar - HalfMap];
      if (info != colo) {
        if (info == bitz) {
  ...
  public static int[] BuildMap(String theList, int ImDims,
    ...
    if (!frax) {
      return null; // if (theList == "")
    }
    if (lino < 3) {
      return null; // too few lines to be real
    }
    ...
    if (nxt > 0) {
      yx = GotImgOps(theList); // >0: has paint, <0: other ims
    }
    if (yx != 0) {
      while (true) { // we have managed art or paint..
        ...
        if (yx > 0) {
          theList = HandyOps.Substring(0, yx + 2, theList);
        }
        if (kx > 0) {
          if (kx < yx) {
            ...
            for (lino = 1; lino <= 99; lino++) {
              ...
              if (HandyOps.SafeParseInt(aStr) != lino) {
                break;
              }
              zx = -zx;
            } //~for // net effect: zx++
        ...
        if (nCells > 0) {
          ...
          for (lino = 5; lino <= 15; lino++) {
            ...
            if (kx == 0) {
              continue; // this sequence is unused
            }
        ...
        // else if (tall<16) ImDims = 0;
          ...
          thar = nImgs + nuIxBase + GridSz + 2; // -> images
        }
        if (thar > 0) {        // nuIxBase = ArtBase+3;
          if (PaintIx > 0) {
            thar = xcell + PaintIx + thar; // xcell = GridSz>>3;
          }
        ...
        if (!Mini_Log || !NoisyMap) {
          ...
          if (xStr.length() > 77) {
            xStr = aStr + HandyOps.Substring(0, 72, xStr)
                + "..'";
          } else {
            xStr = aStr + xStr + "'";
          }
          ...
          for (lino = here + 1; lino >= 0; lino += -1) { // copy/reformat pixels & clear index..
            ...
            if (lino >= thar) {
              if (lino < here) {
                yx--;
                if (yx >= 0) {
                  if (yx < theImgs.length) {
                    colo = theImgs[yx];
                    if (colo < 0) {
                      colo = -1; // transparent
                    } else if (colo < 0x01000000) {
                      colo = colo << 8; // (Tiff file format)
                    } else {
                      colo = -1;
                    }
                  }
    ...
    for (lino = 0; lino <= 32; lino++) {
      DidCells[lino] = 0; // done with this
    }
    nCells = 0;
    if (myMap == null) {
    ...
    here = nuIxBase;
    if (here > 0) {
      if (here < myMap.length - 1) {
        myMap[here - 1] = 0; // end of artifact list
        if (NoisyMap) {
          myMap[here] = 0xDEADBEEF;
        }
      } //~if // visual start of map in log
    }
    if (myMap.length < 16) {
      return null; // last early exit
    }
    ...
    for (lino = 2; lino <= 999; lino++) {
      if (Mini_Log) {
        if (NoisyMap) {
          if (xCh != '\0') // prev'ly unlogged lines..
          {
      ...
      switch (xCh) { // convert gentle curve specs to straight line segments..
        ...
        case 'R': // turn right (C-wise, inside)
          rpt--; // rpt = -2
          if (prio == 0) {
            break; // gotta have a previous endpoint
          }
          nxt = aim + 1;
          if (xCh == 'L') {
            nxt = nxt + 4;
        ...
        case 'Z': // insert segments for standard curve..             // doing 'Z'
          ...
          rpt--; // rpt = -9
          if (rx <= 0x010101) {
            break; // gotta have a defined trajectory
          }
          rpt--; // rpt = -10
          if (rx >= 0xFFFFFF) {
            break;
        ...
        case 'W': // westward         // Gentle Curve (*: use new direction)..
          ...
          if (rx <= 0) {
            break;         // Outside: 0,0 +4+1 +3+3* +1+4* net +8+8
          }
          rpt--; // rpt = -15
          if (cx <= 0) {
            break;
          ...
          rpt--; // rpt = -16
          if (((prio ^ thar) & 0x3FF83FF8) == 0) {
            break; // but not to same cell
          }
          if (false) {
            ...
            yx = zx >> 16;
            if (yx < rx) {
              zx = (zx | 0x70000) + 0x10000;
            } else if (yx > rx) {
              zx = (zx & -0x80000) - 0x10000;
            }
          } //~if
          EW = true;
          aim = 3;
          ...
          // if (zx==thar) break;
          if (prio == thar) {
            break; // end=bgn OK only if dif't facing edge
          }
          // recell = 0;
          kx = prio >> 16;
                            ...
                            HandyOps.Dec2Log(" = ", tall, HandyOps.Dec2Log("/", wide,
                                HandyOps.Int2Log(" (", prio, HandyOps.Flt2Log(") ", whar,
                                    HandyOps.Flt2Log(" --> ", Vat, HandyOps.Flt2Log("/", Hat,
                                        HandyOps
                                            .Flt2Log(HandyOps.IffyStr(Vinc < 0, " ", " +"), Vinc,
                                                HandyOps
                                                    .Flt2Log("/", Hinc, HandyOps.TF2Log(" ", frax,
                                                        HandyOps.Int2Log("\n      __ (", thar,
                                                            ") -- '" + aLine
                                                                + "'"))))))))))))))))));
            aLine = "";
          ...
          for (zx = 0; zx <= yx; zx++) {
            ...
            Vat = Vat + Vinc;
            Hat = Hat + Hinc;
            if (((xcell ^ recell) & 0x3FF83FF8) == 0) {
              continue;
            }
            here = (prio ^ recell) & 0x3FF83FF8; // =0 if now doing end cell
            if (here == 0) {
              nxt = -1; // do this cell then stop
            } else if (nxt > 0) {
              break; // already did end cell in major dir'n
            } else if (EW) {
              if ((here & 0xFFFF) == 0) {
                nxt++;
              }
            } //~if // major dim'n hit..
            else if ((here & -0x10000) == 0) {
              nxt++; // ..so stop unless next matches
            }
            rpt = 0;
              ...
              for (kx = -1; kx <= 1; kx++) {
                ...
                if (((dont ^ recell) & 0x3FF83FF8) == 0) {
                  ...
                  for (rx = rx; rx >= 0; rx += -1) {
                    ...
                    if (((DidCells[rx] ^ recell) & 0x3FF83FF8) != 0) {
                      continue;
                    }
                                ...
                                HandyOps.Hex2Log(" = x", colo, 8, HandyOps.Dec2Log(" @ ", thar,
                                    HandyOps.Int2Log(" ", here, HandyOps.Dec2Log("/", nxt,
                                        HandyOps.Dec2Log(" ", wait, HandyOps.Dec2Log(" ", kx,
                                            HandyOps.Dec2Log(" #", nCells,
                                                HandyOps.IffyStr(info == 0, "",
                                                    HandyOps.Int2Log(" ", info,
                                                        HandyOps.IffyStr(rpt == 0, "",
        ...
        case 'U': // painted (line) spec..
          ...
          rpt = -83;
          if (rx <= 0) {
            break; // words 2&3 are V&H coords, in 2m grid units
          }
          rpt--; // rpt = -84
          if (cx <= 0) {
            break;
          ...
          rpt--; // rpt = -85
          if (nxt < 0) {
            break; // word 4 is option bits..
          }
          if (nxt > 7) {
            break; // currently only 90x rotation & hi-res
          }
          ...
          if (kx <= 0) {
            ...
            rpt--; // rpt = -87
            if (zx < 0) {
              break; // invalid line+ in index line
            }
            if ((tall == 0) || (wide == 0)) {
          ...
          // else if (recell+tall>ImHi) break; // off-image..
          else if ((tall - 1) * ImgWi + thar > ImSz) {
            break; // also checks optn=0 (+wide)
            ...
            break; // rpt = -91: ignore more than one (only accept first)
          }
          rpt = -1;
          aWord = "";
          break; // case 'U'
        case 'T': // start up RealTime Base..
          ...
          if (zx > 0) {
            aStr = HandyOps.Hex2Log(" 0x", (zx << 16) + (yx & 0xFFFF), 6,
                " 0 0 0 0 0 4");
          } else {
            aStr = HandyOps.Dec2Log(" ", yx, " 0 0 0 0 0 4");
          }
          ...
          if (frax) { // allow for fractional position, but add test bits..
            ...
            aWord = aWord + HandyOps.Dec2Log(" ", (nxt << 9) + cx, aStr);
          }
        ...
        case 'J': // macro to build animation sequence..
          if (!fini) {
            continue; // time-based objects at end
          }
          rpt = -24;
          if (ImDims == 0) {
            break;
          ...
          rpt--; // rpt = -27
          if (nxt < 5) {
            break;  // word 4 is sequence+ (5-15), word 5 is + steps
          }
          if (nxt > 15) {
            break;
          }
          ...
          if (kx > 99) {
            break;   // words 6&7 are 1st & last StopInfo ln+s
          }
          ...
          if (whar < 0.0) {
            break; // we don't do negative time..
          }
          if (Vinc < 0.0) {
            break;
          }
          rpt--; // rpt = -31
          if ((whar == 0) != (Vinc == 0)) {
            break; // end frame cannot be inc'd
          }
          xcell = 0;
          if (false) {
            ...
            if (zx > 0) {
              aim = HandyOps.SafeParseInt(HandyOps.RestOf(zx + 1, aStr));
            }
          } //~if
          else {
            aim = 0;
        ...
        case 'Y': // macro to insert artifact from standard descriptor
          ...
          EW = (kx < 5) || (kx > 15);
          if (EW == fini) {
            break; // all&only +5-15 are time-based
          }
          rpt--; // rpt = -37
          nxt = HandyOps.SafeParseInt(HandyOps.NthItemOf(false, 5, aLine));
          if (nxt < 0) {
            break;  // word 5 is facing, c-wise degrees from north
          }
          ...
          if ((kx < 4) || (kx > 15)) {
            ...
            if (nxt < 180) {
              rpt = nxt + 180; // back side
            } else {
              rpt = nxt - 180;
            }
            if (nxt < 270) {
              yx = nxt + 90; // S-edge side
            } else {
              yx = nxt - 270;
            }
          ...
          if (kx == 0) {
            ...
            for (zx = 1; zx <= 99; zx++) { // add all linked lines..
              ...
              if (HandyOps.SafeParseInt(HandyOps.RestOf(yx + 1, aStr)) != kx) {
                break;
              }
              yx = HandyOps.NthOffset(0, "+", aStr);
            ...
            rpt = -41;
            if (nxt < 11) {
              break; // +5 is line+ in StopInfo => (items +5-9)
            }
            rpt--; // rpt = -42
            if (nxt + 2 > AfterStop) {
              break;
            ...
            if (zx < 0) {
              break; // rpt = -42 (invalid line+ in index line)
            }
            ...
            if (nxt >= 0) {
              ...
              if (aStr == "") {
                aStr = HandyOps.RestOf(nxt, aWord);
              }
            ...
            if (zx >= 0) {
              xStr = HandyOps.Substring(0, zx + 1, xStr);
            }
            zx = HandyOps.NthOffset(0, " 0 ", xStr);
            if (zx >= 0) {
              if (zx < 8) {
                xStr = HandyOps.RestOf(zx + 2, xStr);
              }
            }
            if (nxt < 270) {
              yx = nxt + 90; // left side
        ...
        case 'O': // artifact info (see ShoArtif)..
          ...
          rpt--; // rpt = -59
          if (nxt > 359) {
            break; // +4: view angle, 0= centered facing north
          }
          // xcell = 0;
          ...
          if (kx < 0) {
            ...
            kx = 0;    // +11: seq list ptr, +12: exp'n time..
          } else if (kx > 4) {
          ...
          if (kx != 4) {
            ...
            sofar = sofar + 2; // (start RealTime Base needs only +2 words)
          }
          xCh = '\0'; // (already logged)
          continue; // case 'O'
        ...
        case 'X': // erase overstrike memory
          ...
          if (rx > 0) {
            wait = rx; // disable kitty-corner fill (wait) advances
          } else {
            wait = 0;
          }
          if (wait == 0) {
            prev = 0; // enable kitty-corner fill close to prev end
          }
          nCells = 0;
          break; // case 'X'
        case 'P': // pilaster
        ...
        case 'B': // bucket fill
          ...
          if (nxt == 0) {
            colo = 0x66666666; // 0 -> floor
          } else {
            colo = 0x44444444;      // 1 -> track
          }
          TempStr = "(Bkt) '" + aLine + "'";
          TmpI = TempStr.length();
        ...
        case 'H': // E-W
          ...
          if (colo < 4) {
            break; // single thinkness, go do it
          }
          colo = colo & 3;
          recell = nxt;
          nCells = 0;
      ...
      for (rpt = rpt; rpt >= 0; rpt += -1) {
        ...
        if (recell != 0) {
          if (here < myMap.length - (HalfMap + 2)) {
            myMap[here + 1] = colo;
    ...
    nxt = 0;
    if (myMap != null) {
      if (myMap.length > ArtBase + 3) {
        here = myMap[ArtBase + 2] + 2; // needs +3..
      }
      if (here > 8) {
        if (PaintIx > 0) {
          if (here < myMap.length) {
            if (myMap[here] == 0) {
              for (thar = here; thar >= here + 6 - PaintIx;
                  thar += -3) {         // shouldn't happen
                if (myMap[thar] != 0) {
                  break;
                }
    ...
    if (NoisyMap) {
      ...
      if (myMap != null) {
        ...
        if (prio > 0) {
          if (prio < myMap.length - 1) {
            if (myMap[prio] == 0xDEADBEEF) {
              myMap[prio] = 0;
            }
          }
        }
  ...
  public static double MidAngle(double here, double thar) {
    double aim;
    while (here < 0.0) {
      here = here + 360.0; // normalize them..
    }
    while (here > 360.0) {
      here = here - 360.0;
    }
  ...
  public void SetCloseTop(int here) {
    ...
    while (true) {
      ...
      why++; // why = 6 (commit)
      TopCloseView = here;
      if (((xCloseUp - 2) & 0xFF) < 4) {
        nx = MapWiBit - (xCloseUp & 7);
      } else if (TurnRadius < 7.5) {
        nx = 4;
      } else {
        nx = 5; // = log2(displayed close-up width in park meters)
      }
      ...
      if (WhitLnSz == 0.0) {
        ...
        ZooMapWhLn = 2.8 / ZooMapScale; // 1pix = white line width (meters) in c-u
      }
      why = 0;
      break;
    } //~while
  ...
  public int ZoomMap2true(boolean aim2, int rx, int cx) { // rtns (r,c)<<6
    ...
    if (aim2) {
      while (true) { //    Hat = MyMath.Fix2flt(MyMath.SgnExt(rez),6);
        Vat = MyMath.Fix2flt(rez & -0x10000, 22) - Vposn;
        Hat = MyMath.Fix2flt(MyMath.SgnExt(rez), 6) - Hposn;
        if (MyMath.fAbs(Vat) + MyMath.fAbs(Hat) < 1.5) {
          break;
        }
  ...
  public int ZoomMapCoord(boolean yx, double Vat, double Hat) {
    ...
    while (true) {
      ...
      why++; // why = 5
      if (ZooMapScale == 0.0) {
        break; // ZMS*(2m grid) -> img pix in close-up
      }
      why++; // why = 6
      if (Vat < 0.0) {
        break;
      ...
      if ((zx & 0x80008000) != 0) {
        nx = 0; // above or left       // .. <-> meters
      } else if (((ZooMapDim - zx) & 0x80008000) != 0) {
        nx = 0; // below or right
      } else {
        nx = kx + ZooMapBase - (ZooMapTopL << (ZooMapShf));
      }
      if (nx != 0) {
        why = 0; // else why = 13/16
      }
      break;
    } //~while
    return nx;
  ...
  public void ShoDigit(int whom, int rx, int cx, int colo) {
    ...
    for (ro = 4; ro >= 0; ro += -1) {
      for (co = 3; co >= 0; co += -1) {
        if ((bitz & 1) != 0) {
          if (PixScale > 0) {
            RectFill(colo, rx - PixScale, cx - PixScale, rx, cx);
          } else {
            PokePixel(colo, rx, cx);
          }
        } //~if
        cx = cx - 1 - PixScale;
        bitz = bitz >> 1;
  ...
  public void LabelScene(String aLine, int rx, int cx, int colo) {
    ...
    char xCh;
    if (cx < 0) {
      cx = ((more * doit) << 2) - cx; // H-posn<0 places left end of number
    }
    cx = (more << 2) + cx;
    for (nx = doit - 1; nx >= 0; nx += -1) {
      ...
      if (xCh == '-') {
        xCh = '/'; // for now, 'cuz we need '-' but not '/'
      }
      bitz = (int) xCh;
      if (xCh > 'A') {
        switch (bitz & 7) {
...
//        System.out.println(HandyOps.Dec2Log(" (DrRedX) o", OpMode, ""));
  ...
  public void DrawSteerWheel(int posn, boolean scaled, boolean dash2) {
    ...
    if (DrawDash > 0) {
      if (dash2) { // show dashboard..
        if (OpMode == 3) {
          bitz = 0xCC0000; // red when crashed,
        } else if (MyMath.iAbs(SpecWheel) > 88) {
          bitz = 0xFF; // blue when crimped
        }
        ...
        if (Log_Draw) {
          LefDash = LefDash + " t=" + myDash;
        }
        myDash = HandyOps.Flt2Log(" ", Velocity * dFtime, " "); // (5/fps) dFtime=5
        LabelScene(myDash, SceneTall - 4, myDash.length() * 2 + SteerMid, 0xFF9999);
        if (Log_Draw) {
          myDash = " '" + LefDash + "/" + myDash + "'";
        }
      } //~if // (dash2)
    }
    if (scaled) {
      if (posn < 127) { // scale it to TapedWheel..
        info = MyMath.iAbs(posn);
        if (info != 0) {
          for (nx = (TapedWheel[0] & 255) - 1; nx >= 0; nx += -1) {
            if (nx > 0) {
              if (((TapedWheel[nx & 15] >> 8) & 255) > info) {
                continue;
              }
            }
            if (posn < 0) {
  ...
  public boolean NewGridTbl(int[] grids) { // to replace default screen grid..
    ...
    while (true) {
      why++; // why = 1
      if (grids == null) {
        break; // why = 1: null table
      }
      why++; // why = 2
      lxx = grids.length;
      if (lxx < 4) {
        break; // why = 2: not a useful size
      }
      why++; // why = 3
      thar = grids[0];
      if (thar < 2) {
        break; // why = 3: not enough division sets
      }
      why++; // why = 4
      fini = thar - 1;
      for (here = 1; here <= fini; here++) {
        ...
        why++; // why = 5
        if (nx > thar) {
          break; // why = 5: table index out of order
        }
        why++; // why = 6
        prio = 0;
        for (nx = nx; nx <= thar; nx++) { // null division set OK
          ...
          if (info < prio) {
            break; // why = 6: group item out of order (null OK)
          }
          prio = info;
        } //~for
        if (why > 4) {
  ...
  public void SimStep(int mode) {
    ...
    if (nuly < 0) {
      if (DriverCons.D_StartInCalibrate) {
        if (SpecWheel == 0) {
          unScaleStee = true; // used to calibrate servos
        }
      }
      nuly = 0;
    } //~if
...
//                                HandyOps.TF2Log(" Fx=", SimSpedFixt, HandyOps.TF2Log(" Tr=",
// SimInTrak,
...
//        );
  ...
  private int ViewAngle(int locn, int info) { // rtn >0 if in view, <0 if not
    ...
    if (Vx < 6) {
      Vx = 360; // view range < 6 degs, assume 360
    }
    Vx = Vx >> 1;
    Hx = (info & 0xFFF) - aim; // view angle, 0= center facing as spec'd (N=0)
    while (Hx > 180) {
      Hx = Hx - 360; // ..so Hx is angle from center to right
    }
    while (Hx + 180 < 0) {
      Hx = Hx + 360; // ..=angle from mid-view (f.north)
    }
      ...
      while (true) {
        ...
        if (anim > 4) { // one-shot indexed movable stuff (setup @ [%]f)..
          ...
          if (NoisyMap) {
            if (Anim_Log > 0) {
              if ((Anim_Log >> 16) == anim) {
                if ((Anim_Log & 0xFFFF) > FrameNo) {
                  logz = GoodLog; ///¥ change to Mini_Log;
                }
              }
            }
          }
          ...
          while (true) { // fetch img ref(w/locn) from anim'n list by frame +..
            ...
            why++; // why = +7
            if (kx > 0) {
              if (kx < RealTimeNow) { // if (RealTimeNow>0)
          ...
          nx = 0;
          if (whar > 0) {
            if (whar < theInx.length - 1) {
              nx = theInx[whar + 1]; // [=1]
            }
          }
          why++; // why = 33
          if ((nx & -0x10000) > TimeBaseSeq) {
            break;
          }
          ...
          if (zx < 0) {
            ...
            if (GoodLog) {
              System.out.println(HandyOps.Dec2Log("** TimBase ", FrameNo,
          ...
          why++; // why = 14
          if (whar < ArtBase) {
            break; // whar should -> blanked-out light
          }
          why++; // why = 15
          // if (((whar-ArtBase)&3) !=0) break; // (obsolete)
          skipper = true;
          if (ClockTimed) {
            zx = HandyOps.TimeSecs(false);
          } else {
            zx = FakeRealTime >> 10; // FRT in ms
          }
          zx = zx & ((2 << xTrLiteTime) - 1);
          why++; // why = 16
          for (nx = 0; nx <= 9; nx++) {
            whar = whar + 4;
            if (whar >= theInx.length) {
              break; // why = +16
            }
            why++; // why = 17
            info = theInx[whar];
            if (info == 0) {
              why++; // why = +18 (locn = -2)
            } else if (((seen - info) & 0x0FFFFFFF) != 0) {
              why = why + 2; // why = +19
              ...
              locn = info & 0x0FFFFFFF; // why = +17 (good, 1st anim+1)
            }
            break;
          } //~for
          kx = (1 << xTrLiteTime) - 2;
          if ((((zx >> xTrLiteTime) + anim) & 1) == 0) {
            kx = 8; // red
          } else if ((zx & kx) == kx) {
            kx = 4; // yellow
          } else {
          ...
          if (locn > 0) {
            ...
            if (TmpI == 0) {
              locn = locn - 0x80000000;
            }
          }
        } //~if // (traffic light)
        else {
          locn = -1; // if (anim==1) // animation skipover (unused phases)
        }
        break;
      } //~while
    }
    ...
    if (skipper) {
      if (TmpI == 0) {
        for (nx = 9; nx >= 0; nx += -1) {
          whar = whar + 4;
          if (whar < theInx.length) {
            if (((theInx[whar] - seen) & 0x0FFFFFFF) == 0) {
              continue;
            }
          }
          TmpI = (whar << 16) + whar;
                    ...
                    HandyOps.Dec2Log(" #", FrameNo, HandyOps.IffyStr(anim == 0, "",
                        HandyOps.Int2Log(" k=", kx, HandyOps.Int2Log(" z=", zx,
                            HandyOps.Int2Log(" n=", nx, HandyOps.Int2Log("/", TimeBaseSeq,
                                HandyOps
                                    .Fixt8th(" t=", RealTimeNow, HandyOps.Dec2Log("/", FakeRealTime,
                                        HandyOps.TF2Log(" ", ClockTimed,
                                            HandyOps.Int2Log(" -> ", TmpI,
                                                HandyOps.IffyStr(TmpI == 0, aLine,
                                                    HandyOps.Int2Log("\n    ", seen,
                                                        HandyOps.TF2Log(" ", skipper,
                                                            HandyOps.Int2Log(" ", here,
                                                                HandyOps.Dec2Log(" o", OpMode,
                                                                    HandyOps.TF2Log("/", StepOne,
                                                                        HandyOps.Dec2Log(" ",
                                                                            xTrLiteTime,
                                                                            " ---> " + HandyOps
                                                                                .ArrayDumpLine(
...
//                                        HandyOps.Flt2Log(" ", fudge, HandyOps.Dec2Log(" o",
// OpMode,
//                                                HandyOps.TF2Log("/", StepOne, HandyOps.Fixt8th
// (" t=", RealTimeNow,
...
//                                                        ))))))))))))));
  ...
  public int MapColor(int optn, double Vat, double Hat, double ink) {
    ...
    while (true) { // ..so caller can branch off to build vertical walls
      ...
      why++; // why = 3
      if (car2) {
        while (true) { // CarTest, Vcarx,Hcarx,Lcarx,Rcarx,Tcarx,Bcarx,
          car2 = tript;
          whar = (rx << 16) - CarTest + cx; // = 0..5,0..5 if near/on car
          if ((whar & 0x80008000) != 0) {
            break; // above or left (negative)
          }
          why = why + 8; // why = +8 => 15/16
          if (((0x50005 - whar) & 0x80008000) != 0) {
            break; // below or right
          }
      ...
      if (info == 0) { // outside the park..
        if (Wally == 0) {
          break; // still behind last BG
        }
        if (OpMode == 3) {
          if (FrameNo < 2) {
            break; // bad car placement
          }
        }
        if (GroundsColors > 0) {
          break; // no walls if outdoors
        }
        why++; // why = +7
        info++;
      } //~if // inside, so put a wall in front (outside the park)
      else if (info > 0x77000000) {
        info = 4; // (obsolete)
      }
      why++; // why = +7/+8
      if (!flat) {
        if (RectMap > 0) {
          while (true) { // we have paint (see Ad2PntMap)
            if (info >= 0) {
              if (info <= 4) {
                break; // ..but not through walls
              }
            }
            ...
            nx = whar << (cx & 31);
            if (nx >= 0) {
              break; // 'if ((whar<<(cx&31)) >= 0)' failed in T68
            }
            thar = RectMap;
            ...
            while (true) { // look for paint that covers Vat,Hat (3x cont)..
              ...
              why = why + 0x10000; // why = +3,x
              if ((Vx & 0x80008000) != 0) {
                continue; // pix outside bounds rect..
              }
              ...
              if (((Hx - Vx) & 0x80008000) != 0) {
                continue;
              }
              zx = Vx & 0xFFFF;
              Vx = Vx >> 16;
              nx = 0;
              if ((more & 0x03000000) != 0) {
                switch ((more >> 24) & 3) {
                  case 0: // no rotate..
                    Hx = zx;
                    break;
      ...
      if (info < 0) { // pavement edge crosses this square..
        ...
        if ((info & 0x20000000) == 0) { // H-line..
          ...
          if ((info & 0x40000000) == 0) { // > is in
            if (deep > Kconst + ledge) {
              colo = PavColo; // gray
            } else if (deep >= Kconst) {
              colo = 0xFFFFFF;
            }
          } //~if // white line
          else if (deep + ledge < Kconst) {
            colo = PavColo; // gray
          } else if (deep <= Kconst) {
            colo = 0xFFFFFF;
          }
      ...
      else if (info > 0x70000000) {
        colo = -2; // artifact (or tree)
      ...
      // else colo = GrasColo;} //~if // (grass)
        ...
        if (optn > 0) {
          colo = info | -4; // so caller can build vertical walls
        } else if ((info & 1) != 0) {
          colo = CreamWall + tnt; // near-white cream
        } else if (info != 4) {
          colo = DarkWall + tnt; // shadowy gray-tan
        } else if (PilasterCo == 0) {
          colo = DarkWall + tnt;
        } else {
          colo = PilasterCo;
        }
        ...
        colo = -1; // (no BG for now)
      } else {
        colo = DarkWall + tnt;
      }
      if (CheckerBd > 0) {
        if (!flat) {
          if (((rx ^ cx) & CheckerBd) != 0) {
            if (colo == PavColo) {
              colo = PavDk;
            } else if (colo == GrasColo) {
              colo = GrasDk;
            }
          } //~if
        }
    ...
    if (GoodLog) {
      if (tript) { // if (rx>0)
        ...
        if (((colo + 1) & 255) == 0) {
          colo = colo & 0x7F7F9E; // show TripLine in blue..
        } else {
          colo = colo & -2 | 254;
        }
      } //~if
              ...
              HandyOps.Hex2Log(" = x", info, 8, HandyOps.Colo2Log(" -> ", colo,
                  HandyOps.Int2Log(" .. ", whar, HandyOps.Dec2Log("/", thar,
                      HandyOps.Dec2Log("_", RectMap, HandyOps.Flt2Log(" ", Kconst,
                          HandyOps.Flt2Log("/", Mconst, HandyOps
                              .Flt2Log(" ", ledge,
                                  HandyOps.Flt2Log(" ", deep, HandyOps
                                      .Flt2Log("/", whom,
                                          HandyOps.Flt2Log("/", ink,
                                              HandyOps.Int2Log("\n   = ",
                                                  why,    // why =
    ...
    while (true) { // (once through, exit early if error)
      ...
      if (ShowMap) {
        if (DoCloseUp) {
          if (ZooMapDim != 0) {
            while (true) {
              for (nx = RasterMap.length - 1; nx >= 0; nx += -1) {
                RasterMap[nx] = 0.0;
              }
              // ZMD=32,32 ZSf=3 ZB=224,642 ZSc=16. ZW=0.2 ZT=8,16
              if (zx >= MapWy) {
              ...
              if (MyMath.Trunc8(TurnRadius * 2.0) + 4 > yx) {
                step = 2;
              } else {
                step = 1; // in park meters, how much car to show (at edge)
              }
...
//                    HandyOps.Dec2Log(myDash, why, OopsLog)));
  ...
  private void InTrackIt() { // calc new posn & aim to stay centered in track
    ...
    while (true) { // find edges..
      ...
      while (aim < 0.0) {
        aim = aim + 360.0;  // Horz: (whom&0x20000000)==0
      }
      while (aim > 360.0) {
        aim = aim - 360.0;
      }
      ...
      if (whom < 0) { // on edge, which?
        ...
        whom = 0; // whom=0: not starting on any edge
      } else {
        doit = false; // not starting on track
      }
      ...
      for (nx = 5; nx >= -5; nx += -1) { // stepping out 5m to each side, +5m if off-track
        if (ridge <= 0) { // right edge still unseen..
          ...
          if (info == 0) {
            ...
            Hrit = Hlft;
            if (info > 0) {
              ledge = 0; // overshot edge
            } else if (MapColor(5, Vlft, Hlft, 0) == PavColo) {
              ledge = info; // inside
            } else {
              ledge = info & 0x7FFFFFFF;
            }
            whom--;
          } //~if
            ...
            ridge = info & 0x7FFFFFFF; // just past edge
          } else {
            ridge = ridge & 0x7FFFFFFF;
          }
        ...
        if (ledge <= 0) { // left edge still unseen..
          ...
          if (more == 0) {
            if ((whom == 0) && !doit) { // was/still off-track,
              ...
              ledge = ledge & 0x7FFFFFFF; // off-track, prior is best
            }
            ...
            Hlft = Hrit;
            if (more > 0) {
              ridge = 0; // overshot edge
            } else if (MapColor(5, Vrit, Hrit, 0) == PavColo) {
              ridge = more; // inside
            } else {
              ridge = more & 0x7FFFFFFF;
            }
            whom++;
          } //~if
            ...
            ledge = more & 0x7FFFFFFF; // just past edge
          } else {
            ledge = ledge & 0x7FFFFFFF;
          }
        } //~if // prior was best edge
        if (logy) {
          System.out.println(HandyOps.Dec2Log("  .:. ", nx,
              HandyOps.Flt2Log(" L=", Vlft, HandyOps.Flt2Log("/", Hlft,
                  HandyOps.Int2Log(" ", more, HandyOps.Flt2Log(" R=", Vrit,
                      HandyOps.Flt2Log("/", Hrit, HandyOps.Int2Log(" ", info,
                          HandyOps
                              .IffyStr(whom > 0, " + ", HandyOps.IffyStr(whom < 0, " - ", " = "))
                              + HandyOps
                              .IffyStr(ledge < 0, "l", HandyOps.IffyStr(ledge > 0, "L", "."))
                              + HandyOps.IffyStr(ridge < 0, "_r",
                              HandyOps.IffyStr(ridge > 0, "_R", "_,"))))))))));
        }
        if (ledge > 0) {
          if (ridge > 0) {
            break; // stop when both sides found
          }
        }
        if (nx > 0) {
          continue;
      ...
      if (!doit) {
        if (whom == 0) { // no track, no edges ever seen,
      ...
      if (ledge == 0) {
        if (ridge == 0) {
          break; // why = 2: didn't find any edges, don't steer
        }
        why = 5; // saw only right edge
        optn = 4;                                                   // o=04
        if ((ridge & 0x20000000) != 0) {
          optn++;
        }
      } //~if // (N-S)            // o=05
      else if (ridge == 0) {
      ...
      if (logy) {
        aLine = HandyOps.Flt2Log(" K=", Kconst,       // aLine: " M="
      ...
      if (optn == 0) { // why = 3: saw both edges, calculate midline..
        ...
        if (logy) {
          aLine = HandyOps.Flt2Log(aLine, Ccon,           // aLine..
        ...
        if (((ledge ^ ridge) & 0x20000000) != 0) { // incompatible sides..
          ...
          if ((ledge & 0x20000000) != 0) {
            optn = 50; // both Vert       // o=32
          } else {
            optn = 48; // both Horz                            // o=30
          }
      ...
      if ((optn & 0x80) != 0) { // ..probably good..
        if (ftmp > 405.0) {
          Ztmp = Atmp; // ftmp=90: Atmp=aim, =270: Atmp=aim-180
        } else if (ftmp > 315.0) {
          optn = optn | 0x40; // ftmp=0/180: Atmp worthless..
        } else if (ftmp > 225.0) {
          Ztmp = Atmp + 180.0;  // ..(450=360+90) +/-45: good
        } else if (ftmp < 45.0) {
          optn = optn | 0x40;
        } else if (ftmp < 135.0) {
          Ztmp = Atmp;
      ...
      if ((optn & 0x70) == 0) {
        good = Ztmp; // (one edge) 1st alt if straight NG
      } else if ((optn & 0x40) == 0) { // else ignore divergent edges
        if ((ledge | ridge) < 0) {
          good = Ztmp; // some edge is oblique or far
        } else {
          nuly = Ztmp;
        }
      } //~if
      if (logy) {
        if (good != aim) {
          aStr = HandyOps.Flt2Log(" G=", good, " = ");
        } else {
          aStr = " = ";
        }
        if (nuly != aim) {
          aStr = HandyOps.Flt2Log(" N=", nuly, aStr);
        }
    ...
    if (why > 1) { // not steering (or already did), but still moving..
      ...
      while (true) {
        ...
        if (why > 16) {
          if (why < 127) // look ahead an extra step..
          {
            info = MapColor(5, Vposn + Vinc, Hposn + Hinc, 0); // omit paint
          }
        }
        if (logy) {
          System.out.println(HandyOps.Dec2Log("  =.= ", why,
              HandyOps.Flt2Log(" ", nuly, HandyOps.Flt2Log("/", aim,
                  HandyOps.Flt2Log(" => ", Vposn, HandyOps.Flt2Log("/", Hposn,
        ...
        nx = 0;
        if (info == 0) {
          break; // set posn & get out (no test)
        }
        if (info == PavColo) {
          break; // posn is good
        }
    ...
    if (!logy) {
      return; // logy=Mini_Log=F [Last_Minit=T]   // aLine: " M="
    }
    if (aLine == "") {
      aLine = " ";
    }
    ...
    if ((optn & 4) == 0) {
      aLine = HandyOps.Flt2Log(" k=", Kconst,
  ...
  private void MoveCar() { // called once for each frame (not nec'ly sync'sly)
    ...
    while (true) {
      ...
      why++; // why = 3
      if (RampServos) {
        if (ServoStepRate > 0) { // ServoStepRate=0
          if (info > ServoStepRate) {
            info = ServoStepRate;
          } else if (info + ServoStepRate < 0) {
            info = -ServoStepRate;
          }
          if (more > ServoStepRate) {
            more = ServoStepRate;
          } else if (more + ServoStepRate < 0) {
            more = -ServoStepRate;
          }
        } //~if
      }
      ...
      if (SimSpedFixt) {                                      // =FixedSpeed
        if (SpecGas <= 0) {
          ...
          fSpeed = MPF; // fFtime=0.2, fMinSpeed=4.0
        }
      ...
      // else if (GasBrake<MinESCact) fSpeed = 0.0;
        ...
        fSpeed = MyMath.Fix2flt(prev + GasBrake, 1) * fGratio * fFtime;
      }
      ...
      if (!SimSpedFixt) {                                     // !FixedSpeed
        ...
        if (doing == 0.0) {
          doing = Velocity; // doing is in meters/frame
        } else {
          doing = (Velocity + test) * 0.5;
        }
      } //~if // (!SimSpedFixt)
      why = why + 3; // why = 7/8/9
      if (OpMode == 3) {
      ...
      if (OpMode == 0) {
        break;         // SloMotion needs to apply also to Velocity
      }
      ...
      if (ShowMap) {
        ...
        if (thar == 0) {
          break; // adjacent crumbs must be in dif't grid cells
        }
        BreadCrumbs[nCrumbs & Crummy] = here;
        nCrumbs++;
      } //~if
      ...
      try {
        if (OpMode == 2) {
          ...
          if (NextFrUpdate > 0) {
            if (ProcessingTime > 0) {
              while (true) {
                ...
                else if (ProcessingTime + cx > NextFrUpdate) {
                  break;
                } else {
                  cx = HandyOps.TimeSecs(true);
                }
          ...
          if (ProcessingTime == 0) {
            ...
            ProcessingTime = (ProcessingTime + rx) >> 1;
          }
          if (NextFrUpdate == 0) {
            NextFrUpdate = cx + rx;
          } else if (ProcessingTime + ProcessingTime > FrameTime) // FT too fast..
          {
            ...
            NextFrUpdate = NextFrUpdate + FrameTime - ProcessingTime;
          }
        } //~if
        else if (OpMode == 1) {
          MoveCar(); // could crash, so..
        }
        if (NuData > 0) {
          BuildFrame();
        }
  ...
  public boolean StartImage(int rose, int colz, int tile) {
    ...
    while (true) {
      ...
      if (myScreen == null) {
        SetMyScreen(new int[nPixels], rose, colz, tile);
      }
  ...
  public void SetServo(int pin, int set2) {
    ...
    if (!unScaleStee) {
      if (pin == SteerServo) { // because steering is not full-scale
        why = 8;
        info = centered; // (for log)
        if (centered < 0) { // LfDeScaleSt = 90.0/LeftSteer..
          why = 14;
          if (LfDeScaleSt > 1.0) {
            tmp = LfDeScaleSt * valu;
          } else {
            why = 12;
          }
        } else if (centered > 0) {
...
//                                            HandyOps.PosTime(" @ "))))))))));
  ...
  public int GetMapSize() {
    if (TopCloseView == 0) {
      return (MapHy << 16) + MapWy; // in park meters
    }
  ...
  public void Refocus(int newFoc35) { // set new lens focal length (see Zoom35)
    ...
    if (ZoomFocus != 0) {
      if (newFoc35 < 25) {
        return; // ignore unreasonable later requests..
      }
      if (newFoc35 > 250) {
        return;
      }
  ...
  private int Color4ix(int whom) { // only frm InitInd
    ...
    if (Log_Log) {
      System.out.println(HandyOps.Dec2Log(" (Colo4ix) ", why,
  ...
  private boolean InitIndex(int[] theInx) { // true if success, always logs
    ...
    if (theInx != null) {
      if (theInx.length > ArtBase) {
        ...
        MapIxBase = nx;
        if (nx > ArtBase) {
          if (GroundsColors != 0) { // set colors & start pos'n..
            info = theInx[6];
            rx = theInx[5];
            for (nx = nx; nx >= 0; nx += -1) {
              aim = theInx[zx];
            ...
            if (info > 99) {
              ...
              info = (int) Math.round(WhiteLnWi * 100.0);
            }
            if (info >= 0) {
              ...
              if (NumFax > 0) {
                if (TrakImages != null) {
                  ...
                  if (myFax == null) {
                    ...
                    for (info = NumFax * 2 - 1; info >= 0; info += -1) {
                      if (info >= 0) {
                        if (info < myFax.length) {
                          myFax[info] = 0;
                        }
                      }
                    }
    ...
    if (GroundsColors < 0) {
      for (nx = nx; nx <= HalfMap * HalfTall - 1; nx += HalfMap + 1) { // diagonal
        ...
        if (info >= 0x70000000) {
          break; // found BG, it's probly outdoors
        }
        if (info >= 0x40000000) {
          continue;
        }
        if (info <= 4) {
          InWalls = true; // found a wall, it's inside
        }
        break;
    ...
    while (true) { // if ((GroundsColors<0)||(lxx != ParkDims)) // =200<<16 +256
      ...
      why++; // why = 3
      if ((1 << MapWiBit) != MapWide) {
        break; // verify correct constant
      }
      why++; // why = 4
      if (tall >= ImHaf) {
        if (tall + 99 > myHi) {
          break; // ..unless no space for it
        }
      }
      why++; // why = 5
      if ((rx | cx) == 0) {
      ...
      SetCloseTop(lxx);
      if (((ZooMapDim & 0xFF) << ZooMapShf) != MapWide) {
        OK = false;
      }
  ...
  private void ReadTrakIndex() { // only frm LoadTrackIn; see also BuildMap
    ...
    try {
      while (true) {
        ...
        if (info == 0x4C696C45) {
          LilEndian = true; // "LilE"
        } else if (info == 0x456C694C) {
          LilEndian = true;
        } else if (info != 0x42696745) {
          if (info != 0x45676942) {
            break; // "BigE"
          }
        }
  ...
  private void Valid8consts() {
    ...
    info = CheckerBd + 1;
    if (!TrakNoPix) {
      if ((info & -info) != info) // if (DoScenery)
      {
        System.out.println("<!> Masks like CheckerBd only work as 2^n-1");
      }
    }
    if ((MarinBlue & -0x01000000) != 0) {
      aWord = "MarinBlue";
    } else if ((SteerColo & -0x01000000) != 0) {
      aWord = "SteerColo";
    } else if ((CarColo & -0x01000000) != 0) {
      aWord = "CarColo";
    } else if ((DarkWall & -0x01000000) != 0) {
      aWord = "DarkWall";
    } else if ((BackWall & -0x01000000) != 0) {
      aWord = "BackWall";
    } else if ((CreamWall & -0x01000000) != 0) {
      aWord = "CreamWall";
    }
    if (aWord != "") {
    ...
    else if (fMinSpeed > 32.0) {
      System.out.println("<!> MINimum speed 65mph"
  ...
  public void StartPatty(String whom) {
    ...
    if (nClients == 0) {
      ...
      if (TweakRx != 0) {
        while (tRadix > ImHi - 8 - DrawDash) { // TweakRx=0
          if (TweakRx > 0) {
            effTurnRad = effTurnRad + MyMath.Fix2flt(TweakRx, 0); // +TurnRadius/4.0;
            MakeRangeTbl();
          } //~if
          else if (TweakRx < 0) {
            Refocus(ZoomFocus + TweakRx);
          } else {
            break;
          }
        } //~while
