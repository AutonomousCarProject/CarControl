...
public class ScreenCamera extends FlyCamera {
        ...
        public boolean NextFrame(byte[] pixels) { // fills pixels, false if can't (SimCam)
            ...
            BufferedImage bufferImage = rob.createScreenCapture(new Rectangle( 50,50,912,480));
            //int[] pix;
            //pix = bufferImage.getRGB(0,0,912,480,null,0,1);
            //System.out.println(pix.length);
            int[] pix = ((DataBufferInt) bufferImage.getRaster().getDataBuffer()).getData();
            for(int r = 0; r<480;r++){
                for(int c = 0;c<912;c++){
                    pixels[4*912*r+2*c] = (byte) ((pix[r*912 +c])/0x010000);
                    pixels[4*912*r+2*c+1] = (byte) ((pix[r*912 +c]/0x000100)%0x010000);
                    pixels[4*912*r+2*c+2*912] = (byte) ((pix[r*912 +c]/0x000100)%0x010000);
                    pixels[4*912*r+2*c+2*912+1] = (byte) ((pix[r*912 +c])%0x000100);
                }
            }
            return true;
